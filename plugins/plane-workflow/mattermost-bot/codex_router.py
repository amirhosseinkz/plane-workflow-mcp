    ...
    def _build_prompt(self, *, message: str, file_ids: tuple[str, ...], tool_results: list[dict[str, Any]]) -> str:
        ...
        return "\n".join(
            [
                ...
                "For a new task, use create_standard_work_item and include a clear outcome plus observable acceptance_criteria. Ask one concise question when essential information is missing.",
                "For a task update, use update_standard_work_item only with a work_item_id UUID. When the user gives a title or reference instead, choose find_work_items first.",
