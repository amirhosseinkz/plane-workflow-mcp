    ...
    def _build_prompt(self, *, message: str, file_ids: tuple[str, ...], tool_results: list[dict[str, Any]]) -> str:
        ...
        return "\n".join(
            [
                ...
                "Use start_standard_work_item when work begins. Use complete_standard_work_item only for started work and include factual summary and verification; never invent actual time, checks, problems, or follow-ups.",
                "For other task updates, use update_standard_work_item only with a work_item_id UUID. It cannot complete work. When the user gives a title or reference instead, choose find_work_items first.",
